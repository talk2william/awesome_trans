...
-- Load the theme
beautiful.init(os.getenv("HOME") .. "/.config/awesome/themes/Trans/theme.lua")
...
--spacing
 ...
 spacing:set_text(" ")
 --client name
-- client.connect_signal("focus", function(c) n = c.name end)
-- cn = wibox.widget.textbox()
-- client.focus = c
-- cn:set_text("")
...
local strf = '  ' .. '<span color="#333333"> %I:%M  </span>'
...
for s = 1, screen.count() do
    ...
    mytaglist[s] = awful.widget.taglist(s, awful.widget.taglist.filter.all, mytaglist.buttons, nil, list_update)
    ...
    mywibox[s] = awful.wibox({ position = "left", ontop = false, width = "50", screen = s })
    mywibox_ii[s] = awful.wibox({ position = "bottom", ontop = false, height = "30", screen = s })
    ...
    rotation:set_widget(mytaglist[s])
    left_layout:add(rotation)
...
--    middle_layout:add(rotation)

    -- Widgets that are aligned to the right
    local auto_center_layout = wibox.layout.align.horizontal()
    auto_center_layout:set_middle(volume)
    ...
    right_layout:add(auto_center_layout)
    right_layout:add(spacing)
    right_layout:add(spacing)
...
--    layout:set_middle(middle_layout)
    ...
    taskl:set_margins(4)
    local right_layout_ii = wibox.layout.fixed.horizontal()
    right_layout_ii:add(mypromptbox[s])
--    right_layout_ii:add(cn)
    right_layout_ii:add(mypower_opt)
    local layout_ii = wibox.layout.align.horizontal()
    layout_ii:set_middle(taskl)
    layout_ii:set_right(right_layout_ii)
...
--      properties = { icon = "/home/art-attack/.config/awesome/icons/chrome.png" } },
